...
func main() {
	...
	for _, instruction := range instructions {
		...
		if instruction[0] > 1 {
			moveMany(&boxStacks[instruction[1]-1], &boxStacks[instruction[2]-1], instruction[0])
		}
	}
...
func moveMany(a *[]string, b *[]string, amount int) {
	newA := *a
	*b = append(*b, newA[len(newA)-amount:]...)
	newA = newA[:len(newA)-amount]
	*a = newA
}
